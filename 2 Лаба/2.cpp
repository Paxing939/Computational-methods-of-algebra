...
public:
	...
	Matrix() {
		...
		float sum_1_col_k_0 = (float)sum_1_line;
		matrix_k_0_[0][0] = -(float)sum_1_line + 1;
	...
	// функци€ решени€
	std::pair<std::vector<float>, std::vector<float>> Solve() {
		int line_length = matrix_.size();
		// запол€нем матрицу гаусса
		std::vector<std::vector<float>> matrix_transform(line_length);
		std::vector<std::vector<float>> matrix_transform_k_0(line_length);
		...
		for (int k = 0; k < line_length; k++) {
			...
			for (int i = k + 1; i < line_length; i++) {
				float t_k_0 = R(matrix_transform_k_0[i][k] / matrix_transform_k_0[k][k]);
				matrix_transform_k_0[i][k] = t_k_0;
				float t = R(matrix_transform[i][k] / matrix_transform[k][k]);
		...
		// заполн€ем L из D
		std::vector<std::vector<float>> Lt(matrix_.size()), Lt_k_0(matrix_k_0_.size());
		...
		std::vector<float> y(L_.size()), y_k_0(L_k_0_.size());
		...
		std::vector<float> answer(y.size()), answer_k_0(y_k_0.size());
		...
		fout.close();
		return std::pair<std::vector<float>, std::vector<float>> {answer, answer_k_0};
	}

private:
	std::vector<std::vector<float>> matrix_, L_, D_;
	std::vector<std::vector<float>> matrix_k_0_, L_k_0_, D_k_0_;
	std::vector<float> b_, x_, b_k_0_;
};
...
int main() {
	...
	Matrix m;
	std::cout << std::endl;
	std::pair<std::vector<float>, std::vector<float>> pair = m.Solve();
