...
public:
	...
	Polinomial() {
		coeffs_ = { 179000, 56300, -659, -100, 1 };
	...
	void StartIterations() {
		...
		x = (x_1 + x_2) / 2, x_next = 0;
		for (int i = 0; i < 1500; i++) {
			float a = CountPolynomFromX(coeffs_, x);
			float b = CountPolynomFromX(derivative_3, x);
			x_next = x - a / b;
			if (abs(x_next - x) < accurancy_) {
				break;
			}
		...
		for (int i = 0; i < 100; i++) {
			if (OneSign(y_1, y_2)) {
				x_2++;
				y_2 = CountPolynomFromX(coeffs_, x_2);
			}
		}
		x = x_1, x_next = 0;
		for (int i = 0; i < 1500; i++) {
	...
	void Newton() {
		...
		float x = (x_1 + x_2) /2, x_next = 0;
		for (int i = 0; i < 1500; i++) {
			x_next = x - CountPolynomFromX(derivative_3, x) / CountPolynomFromX(derivative_2, x);
			if (abs(x_next - x) < accurancy_) {
				break;
			}
			x = x_next;
		...
		x = (x_1 + x_2) / 2, x_next = 0;
		for (int i = 0; i < 500; i++) {
			float a = CountPolynomFromX(derivative_3, x);
			float b = CountPolynomFromX(derivative_2, x);
				x_next = x - a / b;
			if (abs(x_next - x) < accurancy_) {
				break;
			}
